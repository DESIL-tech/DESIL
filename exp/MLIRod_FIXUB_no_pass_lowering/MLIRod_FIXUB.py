...
class DynamicPassSelector:
    ...
    def random_lowering(self):
        all_lowering_passes = []
        for _ in DynamicPassSelector.lower_map:
            all_lowering_passes.extend(DynamicPassSelector.lower_map[_])
        for _ in DynamicPassSelector.prerequisite_passes:
            all_lowering_passes.append(_)
            all_lowering_passes.extend(DynamicPassSelector.prerequisite_passes[_])
        all_lowering_passes = list(set(all_lowering_passes))
        forbidden_opts = ["all"]
        all_lowering_passes = [_ for _ in all_lowering_passes if _ not in forbidden_opts]
        return all_lowering_passes[random.randint(0, len(all_lowering_passes)-1)]

    def main(self):
        return self.random_lowering()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--init_size", type=int, help="Size of seed pool.", default=3000)
